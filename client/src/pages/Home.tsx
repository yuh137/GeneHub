...
const Home = () => {
  ...
  const disease_data = useLoaderData() as SearchData[];
  ...
  return (
    <>
      <div
        className="first-container relative sm:w-svw sm:h-svh bg-cover"
      ...
      >
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 sm:space-y-4">
          <div className="text-black sm:text-4xl font-semibold text-center mb-1">
            GeneHubAI
          </div>
          <div className="search-filter sm:space-x-4">
            <Dropdown
              ...
              trigger={["click"]}
            >
              <Button className="sm:w-28 sm:min-w-16 sm:h-14 rounded-full text-center hover:!border-[#005555] hover:!text-white hover:!bg-[#005555]">
                {selectedType}
              </Button>
            </Dropdown>
            <input
              type="text"
              className="searchbar my-2 py-2 px-3 sm:w-[28svw] text-[16px] sm:h-14 rounded-full shadow-lg transition duration-75 hover:border hover:border-solid hover:border-[#005555]"
            ...
            />
            <button
              className="sm:w-24 sm:h-14 rounded-full text-center text-sm text-white bg-[#005555] transition delay-150 hover:-translate-y-1"
              onClick={() => {
      ...
      <div className="carousel-container flex flex-col py-8 px-12 sm:space-y-4">
        <div className="flex justify-between px-28">
          <div className="carousel-title self-center sm:text-2xl font-bold">
            Popular Research
          </div>
          <button className="px-2 py-4 rounded-full text-center self-center text-white bg-[#005555] transition delay-150 hover:-translate-y-1">
            View all symptom
          </button>
        </div>
        <Carousel
          autoplay
          infinite
          slidesToShow={width > 640 ? 3 : 1}
          pauseOnHover={false}
          className="sm:w-[80svw] mx-auto"
          ref={researchCarouselRef}
        >
          {disease_data.slice(0, 9).map((item, index) => (
            <div className="carousel-card px-2 sm:min-h-32" key={index}>
              ...
              <div className="card-content shadow-lg px-3 py-4 sm:space-y-6">
                <div className="text-[#005555] sm:text-2xl underline underline-offset-[10px] font-bold truncate">
                  {item.disease_name}
                </div>
                <div className="sm:text-[20px] font-bold">
                  {item.gene}
                </div>
                <div className="sm:space-y-1">
                  {item.symptomps.slice(0, 4).map((sym, index) => (
                    <div key={index} className="text-[#aaa] capitalize">
                      {index + 1}. {sym}
                    </div>
                  ))}
                </div>
                <button
                  className="sm:w-24 sm:h-10 rounded-full text-center text-sm text-white bg-[#005555] transition delay-150 hover:-translate-y-1"
                  onClick={() => {
        ...
        <div className="carousel-buttons flex justify-between px-28">
          <button
            className="sm:w-16 sm:h-10 rounded-full text-center text-sm text-black bg-[#ccc] transition delay-75 hover:bg-[#005555] hover:text-white"
            onClick={() => {
              researchCarouselRef.current?.prev();
            }}
          >
            Prev
          </button>
          <button
            className="sm:w-16 sm:h-10 rounded-full text-center text-sm text-black bg-[#ccc] transition delay-75 hover:bg-[#005555] hover:text-white"
            onClick={() => {
              researchCarouselRef.current?.next();
            }}
          >
            Next
          </button>
      ...
      <div className="researcher-container py-8 px-12 bg-[#F2F3F6] sm:space-y-24 flex flex-col items-center">
        <div className="flex flex-col items-center sm:space-y-4">
          ...
          <div className="sm:max-w-[60%] sm:min-w-1/2 text-center text-[#aaa]">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Excepturi
            repudiandae repellendus debitis nulla officia, dolor a adipisci
            praesentium odio! Aspernatur consequuntur reiciendis explicabo in
            quidem tempore, laudantium fugit quia est.
          </div>
        </div>
        <div className="researcher-cards flex sm:gap-4 sm:gap-y-16 flex-wrap justify-center">
          {feedback_carousel_data.map((item, index) => (
            <div className="researcher-card bg-white px-6 py-10 sm:w-[30%] flex flex-col items-center justify-between relative sm:max-h-[50svh] sm:space-y-6">
              <div
                className="bg-cover sm:w-20 sm:h-20 rounded-full absolute sm:-top-[40px]"
                style={{ backgroundImage: `url(${item.imageSrc})` }}
                key={index}
              />
              <div>
                <div className="researcher-name text-xl font-semibold text-center text-[#294267]">
                  {item.name}
                </div>
                <div className="researcher-role text-center sm:text-sm text-[#aaa]">
                  {item.role}
                </div>
              </div>
              <div
                className="text-[#aaa] text-justify overflow-scroll text-ellipsis"
                ref={(el) => {
                  if (
                    el &&
                    teamRefs.current.length < feedback_carousel_data.length
                  ) {
